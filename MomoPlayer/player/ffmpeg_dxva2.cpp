...
static void dxva2_release_buffer(void *opaque, uint8_t *data)
{
    ...
    //QMutexLocker lk(&w->ctrl->m_dxvaCbMtx);

    DXVA2Context        *ctx = w->ctx;
    int i;

    if (w->ctrl->m_dxvaCbEnabled)
    {
    ...
    {
        //qDebug() << "w->ctrl->m_dxvaCbEnabled false";
        //return;
    }
...
static int dxva2_get_buffer(AVCodecContext *s, AVFrame *frame, int flags)
{
    ...
    if (!ctx)
    {
        qDebug() << "ist->hwaccel_ctx nullptr";
        return -1;
    }
    int i, old_unused = -1;
    LPDIRECT3DSURFACE9 surface;
...
static int dxva2_retrieve_data(AVCodecContext *s, AVFrame *frame)
{
    ...
    //D3DPRESENT_PARAMETERS d3dpp = ctrl->m_d3dpp;
    IDirect3DSurface9 *pBackBuffer = ctrl->m_pBackBuffer;
 ...
 //       pBackBuffer = NULL;
	...
	GetClientRect(ctrl->m_d3dpp.hDeviceWindow, &m_rtViewport);
	ctx->d3d9device->StretchRect(surface, NULL, pBackBuffer, &m_rtViewport, D3DTEXF_LINEAR);
...
static int dxva2_alloc(AVCodecContext *s, HWND hwnd)
{
    ...
    ctrl->m_d3dpp.Windowed = TRUE;
    ctrl->m_d3dpp.BackBufferCount = 0;
    ctrl->m_d3dpp.hDeviceWindow = hwnd;
    ctrl->m_d3dpp.SwapEffect = D3DSWAPEFFECT_DISCARD;
    ctrl->m_d3dpp.BackBufferFormat = d3ddm.Format;
    ctrl->m_d3dpp.EnableAutoDepthStencil = FALSE;
    ctrl->m_d3dpp.Flags = D3DPRESENTFLAG_VIDEO;
    ctrl->m_d3dpp.FullScreen_RefreshRateInHz = D3DPRESENT_RATE_DEFAULT;
    ctrl->m_d3dpp.PresentationInterval = D3DPRESENT_INTERVAL_IMMEDIATE;
	...
	hr = IDirect3D9_CreateDevice(ctx->d3d9, adapter, D3DDEVTYPE_HAL, hwnd, BehaviorFlags, &ctrl->m_d3dpp, &ctx->d3d9device);
    if (FAILED(hr)) {
...
QString errStr(HRESULT hr)
{
    QString ret;
    if (FAILED(hr))
    {
        _com_error err(hr);
        LPCWSTR errMsg = (LPCWSTR)err.ErrorMessage();
        //ret = QString::fromStdWString(errMsg);
        return QStringLiteral("%1").arg(errMsg);
    }
    return ret;
}
...
static int dxva2_create_decoder(AVCodecContext *s)
{
    ...
    if (FAILED(hr)) {
        ...
        qDebug() << QString("Failed to retrieve decoder device GUIDs, %1").arg(err);
        goto fail;
    }
    ...
    for (i = 0; dxva2_modes[i].guid; i++) {
		...
		for (unsigned j = 0; j < target_count; j++)
		{
			...
			if (format) {
                ...
                //qDebug() << QString("%1 is supported for output").arg(format->name);
            }
			else {
				//printf("%d is supported for output (%4.4s)\n", f, (const char*)&f);
                qDebug() << QString("%1 is supported for output").arg((const char*)&f);
            }
		}
    ...
    if (IsEqualGUID(device_guid, GUID_NULL)) {
        ...
        qDebug() << "No decoder device for codec found";
        goto fail;
    }
    ...
    if (!ctx->surfaces || !ctx->surface_infos) {
        ...
        qDebug() << "Unable to allocate surface arrays";
        goto fail;
    }
    ...
    if (FAILED(hr)) {
        ...
        qDebug() << QString("Failed to create %1 video surfaces").arg(ctx->num_surfaces);
        goto fail;
    }
    ...
    if (FAILED(hr)) {
        ...
        qDebug() << "Failed to create DXVA2 video decoder";
        goto fail;
    }
...
int dxva2_init(AVCodecContext *s, HWND hwnd)
{
        ...
        (s->profile & ~FF_PROFILE_H264_CONSTRAINED) > FF_PROFILE_H264_HIGH) 
    {
        ...
        qDebug() << QString("Unsupported H.264 profile for DXVA2 HWAccel: %1").arg(s->profile);
        return AVERROR(EINVAL);
    }
    ...
    if (ret < 0) {
        ...
        qDebug() << "Error creating the DXVA2 decoder";
        return ret;
    }
